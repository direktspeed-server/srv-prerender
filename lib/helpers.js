...
var envy    = require('steal-envy-json')
var Q = require('q');
 ...
 * Setup
 *
 * This is the style and configuration of a steal-ssr Application.
 * returns object with contents of steal-ssr.json and application style
 *
 *   {
...
exports.setup = function(projectPath, env){
  ...
  try{
    ...
    var configPath  = path.join(projectPath, "steal.json")
    var contents    = fs.readFileSync(configPath).toString()
    var publicPath  = path.join(projectPath, "public")
  
  }catch(e){
    try{
      var configPath  = path.join(projectPath, "steal.js")
      var contents    = fs.readFileSync(configPath).toString()
      var publicPath  = projectPath
      configMode = "js"
    }catch(e){
      try{
          var configPath  = path.join(projectPath, "_steal.json")
          var contents    = fs.readFileSync(configPath).toString()
          var publicPath  = projectPath
        }catch(e){
          try{
            var configPath  = path.join(projectPath, "_steal.js")
            var contents    = fs.readFileSync(configPath).toString()
            var publicPath  = projectPath
            configMode = "js"
          }catch(e){
  ...
  // attempt to parse the file
    try{
      ...
      e.stack     = contents
      e.lineno    = -1
      throw new ssr.helpers.stealstealSSRError(e)
    }
