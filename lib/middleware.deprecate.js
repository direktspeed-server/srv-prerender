...
middleware.index = function(dirPath){
  return function(req, res, next){
    ...
    var poly      = ssr.root(__dirname + "/templates");
...
middleware.poly = function(req, res, next){
  ...
  try{
    ...
    if (error.message.split(':')[0] == 'ENOENT') {
      ...
      return ssr.root(__dirname + "/templates").render("error_project_not_found.jade", locals, function(err, body){
      ...
      return ssr.root(__dirname + "/templates").render("error.jade", locals, function(err, body){
        res.statusCode = 500
        res.end(body)
      })
    }
...
middleware.default404 = function(req, res, next){
  ...
  ssr.root(__dirname + "/templates").render("404.jade", locals, function(err, body){
    var type    = helpers.mimeType("html")
    var charset = mime.charsets.lookup(type)
