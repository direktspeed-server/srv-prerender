const
  ...
  ssr         = require('steal-ssr-renderers'),
  Async       = require('async'),
  mime        = require('mime'),
...
module.exports = function stealSSR(){

  if (!(this instanceof stealSSR)) {
    return new stealSSR();
  }
   ...
   * Server
   *
   * Host a single steal-ssr application.
   *
   */
   ...
   * Multihost
   *
   * Host multiple steal-ssr applications.
   *
   */
...
//       middleware.hostProjectFinder(dirPath),
  ...
  this.pipeline = function(root){
    ...
    var terra = ssr.root(publicPath);

    return function(req, rsp, next){
      var normalizedPath  = helpers.normalizeUrl(req.url);
      var priorityList    = ssr.helpers.buildPriorityList(normalizedPath);
      var sourceFile      = ssr.helpers.findFirstFile(publicPath, priorityList);
      ...
      terra.render(sourceFile, function(error, body){
        ...
        var outputType = ssr.helpers.outputType(sourceFile);
   ...
   * Compile
   *
   * Compiles Single steal-ssr Application.
   *
   */
  ...
  this.compile = function(projectPath, outputPath, callback){
    ...
    try{
      ...
      var terra   = ssr.root(setup.publicPath, setup.config.globals);
    }catch(err){
      return callback(err);
    }
     ...
     * Copy File
     *
     * TODO: reference ignore extensions from a ssr helper.
     */
    var copyFile = function(file, done){
      var ext = path.extname(file);
      if(!ssr.helpers.shouldIgnore(file) && ['.jade', '.ejs', '.md', '.styl', '.less', '.scss', '.sass', '.coffee', 'pug'].indexOf(ext) === -1){
